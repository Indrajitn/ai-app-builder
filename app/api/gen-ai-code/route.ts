...
// ─── History trimming and content building ─────────────────────────────────────────────
    ...
    const stream = new ReadableStream({
        async start(controller) {
            ...
            try{
                ...
                let accumulated = ""; // collect the JSON output from the model
                let lastEmitTime = 0; // track last time we emitted a status update
